...
"""Deep neural network for roulette number prediction."""
...
class RouletteNeuralNetwork:
    """LSTM model lifecycle: build → train → save → load → predict."""
    ...
    def build_training_data(history: list):
        """Sliding-window conversion: history → (X, y) tensors for training."""
        ...
        X = np.array(X, dtype=np.float32) / (ROULETTE_NUMBERS - 1)   # normalise → [0, 1]
    ...
    def build_prediction_input(recent: list) -> np.ndarray:
        """Prepare the last SEQUENCE_LENGTH spins for a single prediction call."""
        seq = np.array(recent[-SEQUENCE_LENGTH:], dtype=np.float32) / (ROULETTE_NUMBERS - 1)
